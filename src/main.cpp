...
int main(int argc, char *argv[])
{
    ...
    || true
        ...
        qRegisterMetaType<Packet>();  //可以作为信号与槽的参数
        ...
        if(language == "Chinese")
        {
            ...
            QDEBUG() << "base lang loaded" << translator_qtbase.load(QString("qt_zh_CN"), QLibraryInfo::location(QLibraryInfo::TranslationsPath));
            QDEBUG() << "app lang loaded" << translator.load(":/languages/zh_CN.qm");
            QDEBUG() << QApplication::installTranslator(&translator_qt) << QApplication::installTranslator(&translator_qtbase) << QApplication::installTranslator(&translator) ;
        }
