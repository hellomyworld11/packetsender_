    ...
    , ui(new Ui::MainWindow)
{
    ...
    //绑定响应http响应信号
    QDEBUG() << " http connect attempt:" << connect(http, SIGNAL(finished(QNetworkReply*)), this, SLOT(httpFinished(QNetworkReply*)));

    //绑定 发送包 信号
    QDEBUG() << " packet send connect attempt:" << connect(this, SIGNAL(sendPacket(Packet)), &packetNetwork, SLOT(packetToSend(Packet)));
...
void MainWindow::restorLastSession(QSettings& settings)
{
...
void MainWindow::loadPacketsTable()
{
    ...
    foreach (tempPacket, packetsSaved) {
        ...
        if (tempPacket.name.toLower().contains(filterString) || tempPacket.hexToASCII(tempPacket.hexString).toLower().contains(filterString)
...
void MainWindow::on_udptcpComboBox_currentIndexChanged(const QString& arg1)
{
    ...
    //优化 参数就是combox当前选择值
    QString selectedText = arg1.toLower();
...
void MainWindow::on_generatePanelButton_clicked()
{
    ...
    QDEBUG() << " packet send connect attempt:" << connect(gpanel, SIGNAL(sendPacket(Packet)), &packetNetwork, SLOT(packetToSend(Packet)));
